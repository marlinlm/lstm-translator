...
class Seq2Seq(nn.Module):
    def __init__(self, device, en_word_2_idx, zh_word_2_idx, embeddings, hiddens, n_layers, encoder_dropout, decoder_dropout):
        ...
        self.encoder = enc.BiEncoder(device, src_vocab_size, embeddings, hiddens, encoder_dropout, n_layers)
        self.decoder= dec.Decoder(device, tar_vocab_size, embeddings, hiddens * 2, decoder_dropout, n_layers)
        self.attention = attention.Attention(device, hiddens * 2, hiddens * 2)
        self.linear = nn.Linear(hiddens * 4, tar_vocab_size)
    ...
    def forward(self, x, x_length, y, y_length):
        ...
        # encoder_out: [batch size, seq length, hiddens * 2]
        # hidden, cell: [n layers * 2, batch size, hiddens]
        ...
        eos = torch.Tensor(y.shape[0], 1).to(y.device)
        eos.fill_(self.tar_idx[dict.EOS])
        
        decoder_input = torch.cat((eos, y[:,:-1]), dim=1).long()
        
        # encoder is bidirectional lstm and decoder is single-dir lstm
        hidden = hidden.transpose(0, 1).reshape(hidden.shape[1], -1, hidden.shape[2] * 2).transpose(0, 1)
        cell = cell.transpose(0, 1).reshape(cell.shape[1], -1, cell.shape[2] * 2).transpose(0, 1)
        
        decoder_out, _, _ = self.decoder(decoder_input, y_length, hidden, cell)
        
        attn = self.attention(encoder_out, x_length, decoder_out, y_length)
        attn_out = torch.cat((torch.bmm(attn, encoder_out), decoder_out), dim = -1) 
        
        softmax_out = self.linear(attn_out)
